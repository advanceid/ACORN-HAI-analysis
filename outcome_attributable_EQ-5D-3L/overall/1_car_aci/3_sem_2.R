...
sem_model <- '
  ...
  eq_5d_3l_latent ~ age_new + sex + severity_score_scale + icu_hd_ap + pathogen_combined_types + aci_car
  out ~ eq_5d_3l_latent
'
...
# Coef
important_vars <- c("age_new", "sex", "severity_score_scale", "icu_hd_ap", "pathogen_combined_types", "aci_car")
...
rows <- list(
  ...
  c("Carbapenem-resistant", coef_df[6, c(2:4, 6, 5, 7:10)]))
...
# Save
saveRDS(result, "data/attfbis_table_multi_2.RData")
saveRDS(factor_load_eq, "data/attfbis_factor_load_1.RData")
saveRDS(S_coef_values, "data/attfbis_S_coef_1.RData")