...
table_row <- list(
  ...
  c("Neurological disease", vif_df$vif_values[7]),
  c("Pulmonary disease", vif_df$vif_values[8]),
  c("Others", vif_df$vif_values[9]),
  ...
  c("No", "—"),
  c("Yes", vif_df$vif_values[10]),
  c("Type of pathogens", NA),
  c("Monomicrobial", "—"),
  c("Polymicrobial", vif_df$vif_values[11]),
  c("Infection syndromes", NA),
  c("VAP", "—"),
  c("Hospital-acquired BSI", vif_df$vif_values[12]),
  c("Healthcare-associated BSI", vif_df$vif_values[13]),
  c("Enterobacterales", NA),
  c("Third-generation cephalosporin-susceptible", "—"),
  c("Third-generation cephalosporin-resistant", vif_df$vif_values[14])
)
...
lasso_table <- table_vif %>%
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = 1,
      rows = c(1, 3, 6, 10, 14, 20, 22, 24, 27, 30, 34) 
    )
  ) %>%
  tab_style(
    style = cell_text(
...
# Save
gtsave(lasso_table, filename = "output/table/value_loss_2.html")
###