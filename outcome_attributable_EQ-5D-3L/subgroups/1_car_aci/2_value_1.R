...
resist_var   <- "aci_car"  # resistance variable for Acinetobacter
...
system.time({
  for (i in seq_along(df_used)) {
    ...
    table_row <- list(
      ...
      c("Carbapenem-resistant", vif_df$vif_values[11])
    )
...
if (n_cols == 2) {
  ...
  lasso_table <- lasso_table %>% cols_width(1 ~ px(200), 2 ~ px(60), 3 ~ px(110))
} else {
  lasso_table <- lasso_table %>%
    cols_width(1 ~ px(200), 2 ~ px(60), tidyselect::all_of(3:n_cols) ~ px(110))
}
...
lasso_table <- lasso_table %>%
  ...
  text_transform(
    locations = cells_body(columns = 1, rows = Variables == "Acinetobacter spp."),
