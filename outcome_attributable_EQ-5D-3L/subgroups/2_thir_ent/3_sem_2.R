...
for (i in 1:3){
 ...
 if (i == 1) {
   ...
   # 2. Structural equation model, delete comorbidities_CCI, severity_score_scale, pathogen_combined_types 
   eq_5d_3l_latent ~ age_new + sex + icu_hd_ap + ent_thir
   out ~ eq_5d_3l_latent
  '
 } else if (i == 2) {
   sem_model <- '
   ...
   eq_5d_3l_latent ~ age_new + sex + severity_score_scale + icu_hd_ap + pathogen_combined_types + ent_thir
   out ~ eq_5d_3l_latent
 '
 } else if (i == 3) {
   sem_model <- '
   ...
   # 2. Structural equation model, delete severity_score_scale, icu_hd_ap, pathogen_combined_types
   eq_5d_3l_latent ~ age_new + sex + comorbidities_CCI + ent_thir 
   out ~ eq_5d_3l_latent
  '
 }
  ...
  if (i == 1){
    rows <- list(
      ...
      c("Yes", coef_df[3, c(2:4, 6, 5, 7:10)]),
      ...
      c("Third-generation cephalosporin-resistant", coef_df[4, c(2:4, 6, 5, 7:10)]))
  } else if (i == 2) {
    rows <- list(
      c("Characteristics", NA, NA, NA, "Unstandardized coefficient (95%CI)", "p.value", NA, NA, NA, "Standardized coefficient (95%CI)"),
      c("Age", rep(NA,9)),
      ...
      c("Admission to ICU/HD at enrollment", rep(NA,9)),
      c("No", NA, NA, NA, "Ref", rep(NA,5)),
      c("Yes", coef_df[4, c(2:4, 6, 5, 7:10)]),
      c("Type of pathogens", rep(NA,9)),
      c("Monomicrobial", NA, NA, NA, "Ref", rep(NA,5)),
      c("Polymicrobial", coef_df[5, c(2:4, 6, 5, 7:10)]),
      c("Enterobacterales", rep(NA,9)),
      c("Third-generation cephalosporin-susceptible", NA, NA, NA, "Ref", rep(NA,5)),
      c("Third-generation cephalosporin-resistant", coef_df[6, c(2:4, 6, 5, 7:10)]))
  } else if (i == 3) {
    rows <- list(
      c("Characteristics", NA, NA, NA, "Unstandardized coefficient (95%CI)", "p.value", NA, NA, NA, "Standardized coefficient (95%CI)"),
      c("Age", rep(NA,9)),
...
# Save
saveRDS(result_save, "data/attfbis_table_multi_2_sub.RData")
saveRDS(factor_load_eq_all, "data/attfbis_factor_load_2_sub.RData")
saveRDS(S_coef_values_all, "data/attfbis_S_coef_2_sub.RData")