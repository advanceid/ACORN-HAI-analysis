...
# Data
factor_load_2 <- readRDS("data/attfbis_factor_load_2_sub.RData")
S_coef_2 <- readRDS("data/attfbis_S_coef_2_sub.RData")

factor_load_3 <- readRDS("data/attfbis_factor_load_3_sub.RData")
S_coef_3 <- readRDS("data/attfbis_S_coef_3_sub.RData")

factor_load <- list(factor_load_2[[1]], factor_load_2[[2]], factor_load_3[[3]])
S_coef <- list(S_coef_2[[1]], S_coef_2[[2]], S_coef_3[[3]])
...
for (i in seq_along(S_coef)) {
  ...
  p <- ggplot(S_coef[[i]], aes(y = rhs, x = est.std)) +
    ...
    theme_minimal() +
    scale_x_continuous(limits = c(-0.2, 0.5), breaks = seq(-0.2, 0.4, by = 0.1)) +
...
p <- wrap_plots(plots_combined, ncol = 1, heights = c(0.4, 0.5, 0.35))


# Save
CairoPDF("output/pdf/eq_2_sub.pdf", width = 9, height = 7)
print(p)
dev.off()