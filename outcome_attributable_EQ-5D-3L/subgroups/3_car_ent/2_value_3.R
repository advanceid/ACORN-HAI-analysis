...
system.time({
  for (i in 1:3) {
    ...
    if (i == 1) {
      ...
      # delete "country_region", "hpd_admreason", "icu_hd_ap"
      variables <- c("age_new", "sex", 
                     "country_income",
                     "comorbidities_CCI",
                     "severity_score_scale",
    ...
    if (i == 1) {
      rows <- list(
        ...
        c("Carbapenem-resistant", results[13, c(2:4, 6, 5)])
      )
    } else if (i == 2) {
      rows <- list(
        c("Characteristics", NA, NA, NA, "Adjusted coefficient (95%CI)", "p.value"),
        ...
        c("Admission to ICU/HD at enrollment", NA, NA, NA, NA, NA),
        c("No", NA, NA, NA, "Ref", NA),
        c("Yes", results[9, c(2:4, 6, 5)]),
        c("Type of pathogens", NA, NA, NA, NA, NA),
        c("Monomicrobial", NA, NA, NA, "Ref", NA),
        c("Polymicrobial", results[10, c(2:4, 6, 5)]),
        c("Enterobacterales", NA, NA, NA, NA, NA),
        c("Carbapenem-susceptible", NA, NA, NA, "Ref", NA),
        c("Carbapenem-resistant", results[11, c(2:4, 6, 5)])
      )
    } else if (i == 3) {
      rows <- list(
        c("Characteristics", NA, NA, NA, "Adjusted coefficient (95%CI)", "p.value"),
        ...
        c("Enterobacterales", NA, NA, NA, NA, NA),
        c("Carbapenem-susceptible", NA, NA, NA, "Ref", NA),
        c("Carbapenem-resistant", results[8, c(2:4, 6, 5)])
      )
    }
