...
for (i in 1:3){
 ...
 if (i == 1) {
   ...
   # 2. Structural equation model, delete comorbidities_CCI + severity_score_scale + icu_hd_ap
   eq_5d_3l_latent ~ age_new + sex + pathogen_combined_types + ent_car
   out ~ eq_5d_3l_latent
   '
  ...
  eq_5d_3l_latent ~ age_new + sex + severity_score_scale + icu_hd_ap + ent_car
  out ~ eq_5d_3l_latent
  '
 } else if (i == 3) {
  ...
  if (i == 1){
    rows <- list(
      ...
      c("Polymicrobial", coef_df[3, c(2:4, 6, 5, 7:10)]),
