...
# Thresholds & helper to safely extract VIF
...
system.time({
  for (i in seq_along(df_used)) {
    ...
    # Skip small subgroups
    n_obs <- nrow(df)
    rv_chr <- as.character(df$aci_car)
    n_resist <- sum(
      !is.na(rv_chr) &
    ...
    x.factors <- model.matrix(
      ~ df$sex + df$country_region + df$country_income +
        df$hpd_admreason + df$icu_hd_ap + df$pathogen_combined_types +
        df$aci_car
    )[,-1]
    ...
    # Outcome variable (placeholder)
    y <- factor(df$fbis_score, ordered = TRUE)
    
    # Dummy model for VIF calculation
