...
for (i in 1:2){
  ...
  # MODEL 
  if (i == 1) {
    # Model 1, delete country_income, hpd_admreason, comorbidities_CCI, icu_hd_ap
    formula <- fbis_score ~
      age_new + sex + country_region +  
      severity_score_scale + pathogen_combined_types + 
      aci_car  
  } else if (i == 2) {
    # Model 2, delete country_region, severity_score_scale
    formula <- fbis_score ~
      age_new + sex + 
  ...
  if (i == 1) {
    ...
    rows <- list(
      ...
      c("Acinetobacter spp.", NA, NA, NA, NA, NA),
      c("Carbapenem-susceptible", NA, NA, NA, "Ref", NA),
      c("Carbapenem-resistant", coef_df[6, c(10:13, 9)]))
  } 
