...
for (i in 1:2) {
  ...
  if (i == 1) {
    formula <- fbis_score ~
      age_new + sex + country_region +  
      severity_score_scale + pathogen_combined_types + 
      aci_car 
  } else if (i == 2) {
    formula <- fbis_score ~
      age_new + sex + 
      country_income + icu_hd_ap + 
      aci_car
  } 
  ...
  # For each group, filter based on aci_car values and summarize
  summary_S <- prob_summary(
    df_result[grepl("susceptible$", df_result$aci_car), ], 
    "Carbapenem-susceptible", infection_type
  )
  summary_R <- prob_summary(
    df_result[grepl("resistant$", df_result$aci_car), ], 
    "Carbapenem-resistant", infection_type
  )
...
p <- ggplot(df_plot, aes(x = factor(fbis_score), 
  ...
  theme(
    ...
    axis.title.x = element_text(family = "Times New Roman", size = 10, margin = margin(t = 5)),
    axis.title.y = element_text(family = "Times New Roman", size = 10, margin = margin(r = 5)),
    axis.text.y = element_text(size = 10, family = "Times New Roman", color = "black", margin = margin(r = 10)),
    axis.text.x = element_text(size = 10, family = "Times New Roman", color = "black", margin = margin(t = 3)),
    legend.title = element_markdown(size = 10, family = "Times New Roman"),  # <- Markdown enabled
...
# Save to PDF
CairoPDF("output/pdf/fbis_1_sub.pdf", width = 7, height = 4)
print(p)
dev.off()