...
system.time({
  for (i in seq_along(df_used)) {
    ...
    # skip tiny strata (use ent_thir for 3GCRE)
    n_obs <- nrow(df)
    rv_chr <- as.character(df$ent_thir)
    n_resist <- sum(
      !is.na(rv_chr) &
    ...
    # dummy outcome (only to compute VIF)
    ...
    table_row <- list(
      ...
      c("Third-generation cephalosporin-resistant", get_vif(vif_vals, 11))
    )
...
if (n_cols == 2) {
  lasso_table <- lasso_table %>% cols_width(1 ~ px(165), 2 ~ px(110))
} else if (n_cols == 3) {
  lasso_table <- lasso_table %>% cols_width(1 ~ px(165), 2 ~ px(80), 3 ~ px(120))
} else {
  lasso_table <- lasso_table %>% cols_width(1 ~ px(165), 2 ~ px(60), tidyselect::all_of(3:n_cols) ~ px(125))
}
