...
for (i in 1:3){
  ...
  # MODEL 
  if (i == 1) {
    # Model 1: delete hpd_admreason, comorbidities_CCI
    formula <- fbis_score ~
      age_new + sex + country_region + country_income +
      severity_score_scale + icu_hd_ap + pathogen_combined_types +
      ent_car  
  } else if (i == 2) {
    ...
    # Model 3: delete country_region, country_income, hpd_admreason
    formula <- fbis_score ~
  ...
  if (i == 1) {
    ...
    rows <- list(
      ...
      c("Type of pathogens", NA, NA, NA, NA, NA),
      c("Monomicrobial", NA, NA, NA, "Ref", NA),
      c("Polymicrobial", coef_df[10, c(10:13, 9)]),
      c("Enterobacterales", NA, NA, NA, NA, NA),
      c("Carbapenem-susceptible", NA, NA, NA, "Ref", NA),
      c("Carbapenem-resistant", coef_df[11, c(10:13, 9)])
    )
  } else if (i == 3) {
    rows <- list(
      c("Characteristics", NA, NA, NA, "Adjusted coefficient (95%CI)", "p.value"),
      ...
      c("Enterobacterales", NA, NA, NA, NA, NA),
      c("Carbapenem-susceptible", NA, NA, NA, "Ref", NA),
      c("Carbapenem-resistant", coef_df[7, c(10:13, 9)])
    )
  }
