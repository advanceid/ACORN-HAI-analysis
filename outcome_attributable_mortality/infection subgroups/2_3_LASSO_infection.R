...
# Revise pathogen_combined_types
df_all <- lapply(df_all, function(d) {
  d$pathogen_combined_types <- ifelse(d$pathogen_combined_types == "Polymicrobial",
                                      "Polymicrobial", "Monomicrobial")
  d$pathogen_combined_types <- factor(d$pathogen_combined_types,
...
output_paths <- paste0("output/table/LASSO_att28_infection_",
                       c("01_cra", "02_3gcre", "03_cre", 
                         "04_crp", "05_vre", "06_mrsa", "07_mdr_gnb"), ".html")

# Rows to bold (section titles)
bold_rows <- c(1, 3, 6, 10, 14, 19, 21, 23, 26, 29)
...
for (j in seq_along(bug_labels)) {
  ...
  for (i in seq_along(df_used)) {
    ...
    # ---- skip rules (current subgroup & its resistance var) ----
    n_obs <- nrow(df)
    ...
    x.factors <- model.matrix(
      reformulate(c("df$sex", "df$country_region", 
                    "df$country_income",
                    "df$hpd_admreason", "df$icu_hd_ap",
                    "df$pathogen_combined_types", 
    ...
    table_row <- list(
      ...
      c(sus_label, "—"),
      c(res_label, get_vif(vif_vals, 11))
    )
  ...
  # If all subgroups were skipped, don't write a file
  if (length(table_vif) == 0) {
    message("Skip writing for: ", bug_labels[j], " (all subgroups skipped)")
    next
  }
  ...
  # Build gt table (auto-fit widths; avoids errors when only 1 group remains)
  if (bug_labels[j] == "Enterobacterales (3GCRE)") {
    width_varcol <- px(185)
  } else {
    width_varcol <- px(165)
  }
  ...
  if (n_cols == 2) {
    ...
    lasso_table <- lasso_table %>%
      cols_width(1 ~ width_varcol, 2 ~ px(60), tidyselect::all_of(3:n_cols) ~ px(130))
  }
