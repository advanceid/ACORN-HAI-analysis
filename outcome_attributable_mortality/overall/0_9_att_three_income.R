...
# Plot
# =========================
keep_income  <- c("Overall","High income", 
                  "Upper middle income","Lower middle income")
keep_patho   <- c("amr","mdr","mdr_gnb")
patho_labels <- c(amr = "AMR", mdr = "MDR", mdr_gnb = "MDR-GNB")

df_plot <- death_attr_income %>%
  dplyr::filter(income_level %in% keep_income,
                pathogen %in% keep_patho) %>%
  dplyr::transmute(
    income_level = factor(income_level, levels = keep_income),
    pathogen     = factor(pathogen, levels = keep_patho,
                          labels = patho_labels[keep_patho]),
...
# Colors (must match factor levels)
patho_colors <- c(
  "AMR"     = "#8F9AE6",
  "MDR"     = "#F3A85C",
  "MDR-GNB" = "#4EB3A5"
...
# Format top labels: 1 decimal if ≥0.1, otherwise 2 decimals
fmt_top <- function(x) ifelse(is.na(x), NA_character_,
                              ifelse(x >= 0.1, sprintf("%.1f", x),
                                     sprintf("%.2f", x)))

# Small epsilon to avoid zero on log axis
eps <- 1e-6

# Shared geometry & theme
pd <- position_dodge2(width = 0.75, preserve = "single")

base_plot <- function(dat) {
  ggplot(dat, aes(x = income_level, y = est_m, fill = pathogen)) +
    geom_col(position = pd, width = 0.7) +
    geom_errorbar(aes(ymin = pmax(lo_m, 0), ymax = hi_m),
                  position = pd, width = 0.18, linewidth = 0.6, color = "black") +
    # Top labels
    geom_text(aes(label = fmt_top(est_m),
                  y = pmax(est_m, 0) + (max(dat$hi_m, na.rm = TRUE) * 0.015)),
              position = pd, size = 3, family = "Times New Roman", color = "black") +
...
# ---- Linear axis version (recommended for posters) ----
p_linear <- base_plot(df_plot) +
  scale_y_continuous(
    name   = "Attributable deaths (million)",
    labels = scales::label_number(accuracy = 0.01, suffix = "M"),
    expand = expansion(mult = c(0.02, 0.08))
  ) +
  coord_cartesian(ylim = c(0, NA))  # start at 0

print(p_linear)
ggsave("output/pdf/attrib_deaths_linear.pdf", p_linear,
       width = 6.5, height = 4, device = cairo_pdf)

# ---- Log axis version (optional, for very small values) ----
df_plot_log <- df_plot %>%
  mutate(
    est_m = ifelse(est_m <= 0, eps, est_m),
    lo_m  = ifelse(lo_m  <= 0, eps, lo_m),
    hi_m  = ifelse(hi_m  <= 0, eps, hi_m)
  )

p_log <- base_plot(df_plot_log) +
  scale_y_log10(
    name   = "Attributable deaths (million, log₁₀)",
    labels = function(x) paste0(scales::label_number(accuracy = 0.01)(x), "M"),
    breaks = scales::log_breaks(n = 6),
    minor_breaks = NULL
  ) +
  coord_cartesian(ylim = c(eps, NA), expand = FALSE)

# Uncomment if you want the log version
# print(p_log)
# ggsave("output/pdf/attrib_deaths_log.pdf", p_log,
#        width = 6.5, height = 4, device = cairo_pdf)
