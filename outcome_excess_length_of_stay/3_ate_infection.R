...
# Select covariates using t90 only
...
for (i in seq_along(df_list_t90)) {
  ...
  infections <- levels(df$infection_types)
  
  for (inf in infections) {
    df_sub <- df %>% filter(infection_types == inf)
    ...
    selected_covariates[[paste0(pat_name, "_", inf)]] <- selected_vars
    
    message(paste0("Selected (t90) for Pattern = ", pat_name, 
                   ", Infection = ", inf, ": ", paste(selected_vars, collapse = ", ")))
  }
}
...
for (tc in cutoffs) {
  ...
  for (i in seq_along(df_list)) {
    ...
    for (inf in infections) {
      ...
      n_analyzed <- n_total # Initialize to total N (for capping)
      
      if (n_total >= 100 && n_levels == 2 && n_evt1 >= 30) {
        
        selected_vars <- selected_covariates[[paste0(pat_name, "_", inf)]]
        if (is.null(selected_vars)) next
        ...
        ps_model <- tryCatch({
          ...
          message(paste("PS Model Error:", e$message, "Skipping Capping for", pat_name, inf, tc))
          return(NULL)
        })
        ...
        if (!is.null(ps_model)) {
          ...
          # Apply Capping to the PS score: replace extreme values
          ...
          message(paste0("Capping applied (", tc, ", ", pat_name, ", ", inf, 
        ...
        # cens_formula (uses the full df_sub)
        ...
        res1 <- tryCatch({
          ...
          if (is.null(s) || all(is.na(s))) {
            message(paste("ATE estimate failed post-capping for:", pat_name, inf, tc))
            return(NULL)
          }
          ...
          message(paste("Fatal Error in resmeanATE (Post-Capping) for:", pat_name, inf, tc, "-", e$message))
          return(NULL)
        })
...
df_combined <- bind_rows(results_part1) %>%
  mutate(
    Group = paste0(Infection, " (N = ", Total_N, ")"),
  ...
  select(
    Cutoff, Pattern, Group,
    Estimate_CI, P_value
  )

df_combined$group_by <- "Infection syndromes"

# Save
write.csv(df_combined, "output/excess_los_by_infection_type.csv", row.names = FALSE)
saveRDS(df_combined, "data/excess_los_by_infection_type.RData")